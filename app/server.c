
#include <errno.h>
#include <netinet/in.h>
#include <netinet/ip.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <sys/socket.h>
#include <unistd.h>

int main() {
  // Disable output buffering
  setbuf(stdout, NULL);
  setbuf(stderr, NULL);

  // You can use print statements as follows for debugging, they'll be visible
  // when running tests. printf("Logs from your program will appear here!\n");

  // Uncomment this block to pass the first stage
  //
  int server_fd, client_addr_len;
  struct sockaddr_in client_addr;

  server_fd = socket(AF_INET, SOCK_STREAM, 0);
  if (server_fd == -1) {
    printf("Socket creation failed: %s...\n", strerror(errno));
    return 1;
  }

  // Since the tester restarts your program quite often, setting SO_REUSEADDR
  // ensures that we don't run into 'Address already in use' errors
  int reuse = 1;
  if (setsockopt(server_fd, SOL_SOCKET, SO_REUSEADDR, &reuse, sizeof(reuse)) <
      0) {
    printf("SO_REUSEADDR failed: %s \n", strerror(errno));
    return 1;
  }

  struct sockaddr_in serv_addr = {
      .sin_family = AF_INET,
      .sin_port = htons(4221),
      .sin_addr = {htonl(INADDR_ANY)},
  };

  if (bind(server_fd, (struct sockaddr *)&serv_addr, sizeof(serv_addr)) != 0) {
    printf("Bind failed: %s \n", strerror(errno));
    return 1;
  }

  int connection_backlog = 5;
  if (listen(server_fd, connection_backlog) != 0) {
    printf("Listen failed: %s \n", strerror(errno));
    return 1;
  }

  printf("Waiting for a client to connect...\n");
  client_addr_len = sizeof(client_addr);

  int fd = accept(server_fd, (struct sockaddr *)&client_addr, &client_addr_len);
  printf("Client connected\n");

  // buffer for the incoming request
  char reqBuff[1024];
  recv(fd, reqBuff, sizeof(reqBuff), 0);

  char reqBuffDup[1024];

  // copying reqBuff into reqBuffDup
  strcpy(reqBuffDup, reqBuff);

  printf(reqBuff);

  char *path = NULL;

  path = strtok(reqBuff, " ");
  path = strtok(NULL, " ");
  // printf("path: %s\n", path);

  char *endpoint = strtok(path, "/");
  // printf("endpoint: %s\n", endpoint);

  if (endpoint != NULL && strcmp(endpoint, "echo") == 0) {
    printf("in echo");
    char *echostr = strtok(NULL, "/");
    printf("%s\n", echostr);

    char response[256];

    snprintf(response, sizeof(response),
             "HTTP/1.1 200 OK\r\nContent-Type: text/plain\r\nContent-Length: "
             "%d\r\n\r\n%s",
             strlen(echostr), echostr);

    send(fd, response, sizeof(response), MSG_EOR);
  } else if (endpoint != NULL && strcmp(endpoint, "user-agent") == 0) {
    printf("in user-agent");
    printf("%s\n", reqBuffDup);
    char *user_agent_string = strtok(reqBuffDup, "\n");
    for (int i = 0; i <= 1; i++) {
      user_agent_string = strtok(NULL, "\n");
      // printf("%d%s\n", i, user_agent_string);
    }

    char *user_agent = strtok(user_agent_string, " ");
    user_agent = strtok(NULL, " ");
    printf("%s\n", user_agent);
    printf("length of user_agent: %d\n", strlen(user_agent));

    if (user_agent[strlen(user_agent) - 1] == '\r') {
      user_agent[strlen(user_agent) - 1] = '\0';
    }

    printf("length of user_agent: %d\n", strlen(user_agent));

    char response[256];
    snprintf(response, sizeof(response),
             "HTTP/1.1 200 OK\r\nContent-Type: text/plain\r\nContent-Length: "
             "%d\r\n\r\n%s",
             strlen(user_agent), user_agent);
    send(fd, response, sizeof(response), MSG_EOR);

  } else if (strcmp(path, "/") == 0) {
    send(fd, "HTTP/1.1 200 OK\r\n\r\n", strlen("HTTP/1.1 200 OK\r\n\r\n"),
         MSG_EOR);
  } else {
    send(fd, "HTTP/1.1 404 Not Found\r\n\r\n",
         strlen("HTTP/1.1 404 Not Found\r\n\r\n"), MSG_EOR);
  }

  close(server_fd);
  // if (path != NULL) {
  //   free(path);
  //   path = NULL;
  // }

  return 0;
}
